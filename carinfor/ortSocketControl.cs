...
namespace carinfor
{
    ...
    public class ortSocketControl
    {
        ...
        public bool UploadAsmResult(OrtASMVehicleInfo vehicleinfo, OrtASMResult asmresult, out string Result, out string Err)
        {
            ...
            xmldoc = new XmlDocument();
            xmlelem = xmldoc.CreateElement("", "Connection", "");
            setXmlElementAttribute(xmlelem, "Action", "UploadASMResult");
            xmldoc.AppendChild(xmlelem);
            XmlNode root = xmldoc.SelectSingleNode("Connection");//查找<Employees> 
            ...
            setXmlElementAttribute(xe1, "RegDate", vehicleinfo.RegData);
            ...
            setXmlElementAttribute(xe1, "RatedSpeed", vehicleinfo.RateSpeed);
        ...
        public bool UploadAsmDataseconds(string testID, DataTable dt, out string Result, out string Err)
        {
            ...
            string tickcount = "",speed="",rotary="",ihpwithplhp="",hc="",co="",no="",co2="",o2="",torque="";
            for (int i = 1; i < dt.Rows.Count; i++)
            {
                tickcount += i.ToString() + ",";
                speed += double.Parse(dt.Rows[i]["实时车速"].ToString()).ToString("0.0") + ",";
                rotary += dt.Rows[i]["转速"].ToString() + ",";
                double b = double.Parse(dt.Rows[i]["加载功率"].ToString());
                if (b < 0) b = 0;
                ihpwithplhp +=b.ToString("0.0") + ",";
                hc += dt.Rows[i]["HC实时值"].ToString() + ",";
                co += dt.Rows[i]["CO实时值"].ToString() + ",";
                no += dt.Rows[i]["NO实时值"].ToString() + ",";
                co2 += dt.Rows[i]["CO2实时值"].ToString() + ",";
                o2 += dt.Rows[i]["O2实时值"].ToString() + ",";
                double a = double.Parse(dt.Rows[i]["加载功率"].ToString())*1000 * 3.6 / double.Parse(dt.Rows[i]["实时车速"].ToString());
                if (a < 0) a = 0;
                torque += a.ToString("0") + ",";
            }
            XmlElement xe1 = xmldoc.CreateElement("ASMProcess");//创建一个<Node>节点 
            setXmlElementAttribute(xe1, "TestID", testid);
            setXmlElementAttribute(xe1, "TickCount",tickcount.Remove(tickcount.Length-1));
            setXmlElementAttribute(xe1, "Speed", speed.Remove(speed.Length - 1));
            setXmlElementAttribute(xe1, "Rotary", rotary.Remove(rotary.Length - 1));
            setXmlElementAttribute(xe1, "IHPWithPLHP", ihpwithplhp.Remove(ihpwithplhp.Length - 1));
            setXmlElementAttribute(xe1, "HC", hc.Remove(hc.Length - 1));
            setXmlElementAttribute(xe1, "CO", co.Remove(co.Length - 1));
            setXmlElementAttribute(xe1, "NO", no.Remove(no.Length - 1));
            setXmlElementAttribute(xe1, "CO2", co2.Remove(co2.Length - 1));
            setXmlElementAttribute(xe1, "O2", o2.Remove(o2.Length - 1));//10
            setXmlElementAttribute(xe1, "Torque", torque.Remove(torque.Length - 1));
            root.AppendChild(xe1);
            if (!connectsocket())
            {
                Result = "FALSE";
        ...
        public bool UploadAsmDataseconds(string testID, DataTable dt, out string Result, out string Err)
        {
            ...
            for (int i = 1; i < dt.Rows.Count; i++)
            {
                XmlElement xe1 = xmldoc.CreateElement("ASMProcess");//创建一个<Node>节点 
                setXmlElementAttribute(xe1, "TestID", testID);
                setXmlElementAttribute(xe1, "TickCount", i.ToString());
